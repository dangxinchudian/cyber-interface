...
$rule = $attackModel->db()->query($sql, 'row');
...
if(!$alarm){
	//没有告警需要闭合或者告警已经闭合
	if(count($recent) > 0 && $recent[0]['status'] == 'normal') exit('全部已闭合');
	if(count($recent) == 0) exit('不需要闭合');
	else{       //发送正常告警
...
$day = $attackModel->db()->query($sql, 'row');
